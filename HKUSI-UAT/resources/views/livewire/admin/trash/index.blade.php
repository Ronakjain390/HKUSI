<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card custom-card">
        ...
        <form method="post" action="{{route('admin.trash.multipleusersdelete')}}">
		{{ csrf_field() }}
                ...
                <table class="dt-select-table table dataTable no-footer dt-checkboxes-select " id="DataTables_Table_3 " aria-describedby="DataTables_Table_3_info ">
                    <thead>
                        <tr>
                            ...
                            <th onclick="return shorting('created_at','{{$order_by}}');" class="sorting @if(isset($order_type) && $order_type == 'created_at' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'created_at' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0 " aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1" aria-sort="descending " aria-label="Name: activate to sort column ascending ">Registration Date</th>
                    ...
                    @if(count($trash))
                        @php  $i=1; @endphp        
                            @foreach ($trash as $key => $memberData)
                                <tr>
                                    <td valign="top" colspan="1" class="dt-checkboxes-cell dt-checkboxes-select-all "><input type="checkbox" name='id[]' class="form-check-input" value="{{$memberData->application_number}}"></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.memberDetail',[$memberData->id,'show'])}}">@if(isset($memberData->created_at) && !empty($memberData->created_at)) {{date('Y-m-d' , strtotime($memberData->created_at))}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
