<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card custom-card">
                                                    ...
                                                    <div class="fliter-checkbox">
                                                        <div class="form-check  custom-option-basic">
                                                            <label class="form-check-label custom-option-content" for="customRadioTemp1">
                                                                ...
                                                                <span class="custom-option-header">
                                                                    <div class="input-calender">
                                                                        <input class="form-control" wire:model.defer="start_date" type="date" placeholder="from ">
                                                                    </div>
                                                                    <div class="input-calender">
                                                                        <input class="form-control" wire:model.defer="end_date" type="date" placeholder="from">
                                                                    </div>
                                                                </span> 
                                                            </label>
                                                        </div>
        ...
        <form method="post" action="{{route('admin.event-setting.multipleEventDelete')}}">
        {{ csrf_field() }}
                    ...
                    <div class=" fliter-flex d-flex align-items-center">
                        ...
                        <div class="flex-filter-box">
                            ...
                            <div class="filter-button fx plus-button">
                                <a href="{{route('admin.event-setting.create')}}" >
                ...
                <table class="dt-select-table table dataTable no-footer dt-checkboxes-select " id="DataTables_Table_3 " aria-describedby="DataTables_Table_3_info ">
                    <thead>
                        <tr>
                            ...
                            <th onclick="return shorting('id','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="Position: activate to sort column ascending ">Event #</th>
                            <th onclick="return shorting('event_name','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'event_name' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'event_name' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="Position: activate to sort column ascending ">Event Name</th>
                            <th onclick="return shorting('date','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'date' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'date' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="City: activate to sort column ascending ">Date</th>
                            <th onclick="return shorting('start_time','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'start_time' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'start_time' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="Date: activate to sort column ascending ">Start Time</th>
							<th onclick="return shorting('end_time','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'end_time' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'end_time' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="Date: activate to sort column ascending ">End Time</th>
                        ...
                        @php  $i=1; @endphp        
                            @foreach ($eventData as $key => $eventDetail)
                                <tr>
                                    ...
                                    <a href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}">@if(isset($eventDetail->created_at) && !empty($eventDetail->created_at)) {{date('Y-m-d  H:i:s' , strtotime($eventDetail->created_at))}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}">{{$eventDetail->getYearDetails->year ?? 'N/A'}}</a></td><td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}">@if(isset($eventDetail->id) && !empty($eventDetail->id)) # {{$eventDetail->id}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}">@if(isset($eventDetail->event_name) && !empty($eventDetail->event_name))  {{$eventDetail->event_name}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}">@if(isset($eventDetail->date) && !empty($eventDetail->date)) {{date( 'Y-m-d' , $eventDetail->date)}} @else N/A @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}" >@if(isset($eventDetail->start_time) && !empty($eventDetail->start_time)) {{date('H:i',$eventDetail->start_time)}} @endif</a></td>
									<td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}" >@if(isset($eventDetail->end_time) && !empty($eventDetail->end_time)) {{date('H:i',$eventDetail->end_time)}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                        <a href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}">
                                            @if(isset($eventDetail->unit_price) && !empty($eventDetail->unit_price)) 
                                                ${{number_format($eventDetail->unit_price , 2)}} 
                                                @else Free
                                            @endif
                                        </a>
                                    </td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}">@if(isset($eventDetail->application_deadline) && !empty($eventDetail->application_deadline)) {{date( 'Y-m-d' , $eventDetail->application_deadline)}} @else N/A @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty" >
                                        <div class="select_status_filter">                                            
                                            <span class="badge rounded-pill badge-custom  @if($eventDetail->status=='Enabled') green @elseif($eventDetail->status=='Disabled') brown @elseif($eventDetail->status=='Cancelled') gray @endif"> @if(isset($eventDetail->status) && !empty($eventDetail->status)){{$eventDetail->status}}  @endif</span>
                                        </div>                                      
                                    </td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                        <div class="dropdown text-center">
                                            ...
                                            <div class="dropdown-menu table-dropdown">
                                                <a class="dropdown-item" href="{{route('admin.eventSettingDetails',[$eventDetail->id,'edit'])}}">Edit</a>
                                                <a class="dropdown-item" href="{{route('admin.eventSettingDetails',[$eventDetail->id,'show'])}}">View</a>
