        ...
        <div class="card-datatable dataTable_select text-nowrap table-responsive custom-table ">
            <div id="DataTables_Table_3_wrapper " class="dataTables_wrapper dt-bootstrap5 no-footer ">
                <table class="dt-select-table table dataTable no-footer dt-checkboxes-select " id="DataTables_Table_3 " aria-describedby="DataTables_Table_3_info ">
                    <thead>
                        <tr>
                            <th onclick="return shorting('name','{{$order_by}}');" class="sorting   @if(isset($order_type) && $order_type == 'name' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'name' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " s="" aria-label="Programe Name: activate to sort column ascending ">Country Name</th>
                        ...
                        @if(count($programme))
                            @foreach($programme as $key => $programeData)
                                <tr>
                                    ...
                                    <a href="#">@if(isset($programeData->name) && !empty($programeData->name)) {{$programeData->name}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="#">@if(isset($programeData->reason) && !empty($programeData->reason)) {{$programeData->reason}} @endif</a></td>
                                    <td valign="top" colspan="1" class="right dataTables_empty"><span class="badge rounded-pill badge-custom @if(isset($programeData->status) && $programeData->status == '1') green @else gray @endif">@if(isset($programeData->status) && $programeData->status == '1') Completed @else Failed @endif</span></td>
                                </tr>
                            @endforeach
                        @else
                        <td colspan="2"></td>
