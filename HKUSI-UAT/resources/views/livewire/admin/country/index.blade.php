<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card custom-card">
        ...
        <form method="post" action="{{route('admin.country.multiplecountrydelete')}}">
        {{ csrf_field() }}
                ...
                <table class="dt-select-table table dataTable no-footer dt-checkboxes-select " id="DataTables_Table_3 " aria-describedby="DataTables_Table_3_info ">
                    <thead>
                        <tr>
                            ...
                            <th onclick="return shorting('name','{{$order_by}}');" class="sorting   @if(isset($order_type) && $order_type == 'name' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'name' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " s="" aria-label="Programe Name: activate to sort column ascending ">Name</th>
                    ...
                    <tbody>
                        @if(count($programme))
                            @foreach($programme as $key => $countrydata)
                                <tr>
                                    <td valign="top" colspan="1" class="dt-checkboxes-cell dt-checkboxes-select-all "><input type="checkbox" name='id[]' class="form-check-input" value="{{$countrydata->id}}"></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="javascript:void(0)">@if(isset($countrydata->created_at) && !empty($countrydata->created_at)) {{date('Y-m-d H:i:s' , strtotime($countrydata->created_at))}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="javascript:void(0)">@if(isset($countrydata->name) && !empty($countrydata->name)) {{$countrydata->name}} @endif</a></td>
                                      <td valign="top" colspan="1" class="dataTables_empty" >
                                        <div class="select_status_filter">                                            
                                            <span class="badge rounded-pill badge-custom  @if($countrydata->status=='1') green @else gray @endif"> @if($countrydata->status==1) Enable @else Disable  @endif</span>
                                        </div>                                      
                                    </td>   
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                        <div class="dropdown text-center">
                                            ...
                                            <div class="dropdown-menu table-dropdown">
                                                <a class="dropdown-item" href="{{route('admin.country.edit',$countrydata->id)}}">Edit</a>
                                                <a wire:click="$emit('triggerDelete',{{$countrydata->id }})" class="dropdown-item" href="javascript:void(0);">Delete</a>     
                                                @if(isset($countrydata->status) && $countrydata->status == '0')
                                                <a wire:click="$emit('triggerStatus',{{ $countrydata->id }})" class="dropdown-item" href="javascript:void(0);">Enable</a>
                                                @else
                                                <a wire:click="$emit('triggerStatus',{{ $countrydata->id }})" class="dropdown-item" href="javascript:void(0);">Disable</a>
                                                @endif                                         
                                            </div>
                                        </div>
                                    </td>
