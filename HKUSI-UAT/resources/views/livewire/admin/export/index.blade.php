<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card custom-card">
        <div class="filter-card">
         ...
         <form method="post" action="{{route('admin.export.multipleExportDataDelete')}}">
        {{ csrf_field() }}
        ...
        <div class="card-datatable dataTable_select text-nowrap table-responsive custom-table ">
            <div id="DataTables_Table_3_wrapper " class="dataTables_wrapper dt-bootstrap5 no-footer ">
                <table class="dt-select-table table dataTable no-footer dt-checkboxes-select " id="DataTables_Table_3 " aria-describedby="DataTables_Table_3_info ">
                    <thead>
                        <tr>
                           <th class="sorting_disabled dt-checkboxes-cell dt-checkboxes-select-all " rowspan="1 " colspan="1 " data-col="0 " aria-label=" " ><input type="checkbox" class="form-check-input " id="checkAll"></th>
                            ...
                            <th onclick="return shorting('id','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="Export: activate to sort column ascending ">Export #</th>
                            ...
                            <th onclick="return shorting('id','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="City: activate to sort column ascending ">Records</th>
                    ...
                    <tbody>
                        @if(count($export))
                            @foreach($export as $key => $exportData)
                                <tr>
                                    <td valign="top" colspan="1" class="dt-checkboxes-cell dt-checkboxes-select-all "><input type="checkbox" name='id[]' class="form-check-input" value="{{$exportData->id}}"></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.export.exportDetail',[$exportData->id,'detail'])}}">@if(isset($exportData->created_at) && !empty($exportData->created_at)) {{date('Y-m-d  H:i:s' , strtotime($exportData->created_at))}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.export.exportDetail',[$exportData->id,'detail'])}}">@if(isset($exportData->id) && !empty($exportData->id)) #{{$exportData->id}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.export.exportDetail',[$exportData->id,'detail'])}}">@if(isset($exportData->type) && !empty($exportData->type)) {{$exportData->type}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.export.exportDetail',[$exportData->id,'detail'])}}">@if(isset($exportData->getUserDetail->name) && !empty($exportData->getUserDetail->name)) {{$exportData->getUserDetail->name}} @endif</a></td>
                                    
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                        <a href="{{route('admin.export.exportDetail',[$exportData->id,'detail'])}}"> 
                                            @if(isset($exportData->getExportPaymentInfos) && !empty($exportData->getExportPaymentInfos) && $exportData->type == 'Payment')
                                                {{$exportData->getExportPaymentInfos->count()}}
                                            @elseif(isset($exportData->getExportHallBookingsInfos) && !empty($exportData->getExportHallBookingsInfos) && $exportData->type == 'HallBooking')
                                                {{$exportData->getExportHallBookingsInfos->count()}}
                                            @elseif(isset($exportData->getExportHallBookingsInfos) && !empty($exportData->getExportHallBookingsInfos) && $exportData->type == 'EventBooking')
                                                {{$exportData->getExportEventBookingsInfos->count()}}
                                            @elseif(isset($exportData->getExportPrivateEventBookingsInfos) && !empty($exportData->getExportPrivateEventBookingsInfos) && $exportData->type == 'PrivateEventBooking')
                                                {{$exportData->getExportPrivateEventBookingsInfos->count()}}
                                            @endif
                                        </a>
                                    </td> 
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                        <span class="badge rounded-pill badge-custom @if(isset($exportData->status) && $exportData->status == '1') green @else gray @endif">@if(isset($exportData->status) && $exportData->status == '1') Success  @else Failed @endif</span>
                                    </td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                        <div class="dropdown text-center">
                                            ...
                                            <div class="dropdown-menu table-dropdown">
                                                <a class="dropdown-item" href="{{route('admin.export.exportDetail',[$exportData->id,'detail'])}}">View</a>
                                                <a wire:click="$emit('triggerDelete',{{$exportData->id }})" class="dropdown-item" href="javascript:void(0);">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
