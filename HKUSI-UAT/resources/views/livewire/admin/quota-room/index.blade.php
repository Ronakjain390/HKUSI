<div class="flex-grow-1 container-p-y">
    <div class="card custom-card">
        ...
        <form method="post" action="{{route('admin.room.multipleRoomDelete')}}">
		{{ csrf_field() }}
                    ...
                    <div class=" fliter-flex d-flex align-items-center">
                        ...
                        <div class="flex-filter-box" wire:ignore>
                            ...
                            <div class="filter-button fx plus-button">
                                @if(Request::segment(2) === 'quota-hall')
                                    <a href="{{route('admin.room.roomDetail',[$quota_hall_id,'create'])}}">
                ...
                <table class="dt-select-table table dataTable no-footer dt-checkboxes-select " id="DataTables_Table_3 " aria-describedby="DataTables_Table_3_info ">
                    <thead>
                        <tr>
                            ...
                            <th onclick="return shorting('id','{{$order_by}}');" class="sorting @if(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0 " aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1" aria-sort="descending " aria-label="Name: activate to sort column ascending ">Room #</th>
                            ...
                            <th onclick="return shorting('id','{{$order_by}}');" class="sorting @if(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'id' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0 " aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1" aria-sort="descending " aria-label="Name: activate to sort column ascending ">Room Code</th>
                            <th onclick="return shorting('total_quotas','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'total_quotas' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'total_quotas' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="City: activate to sort column ascending ">Hall/Collage</th>
                            <th onclick="return shorting('quota_balance','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'quota_balance' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'quota_balance' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="Date: activate to sort column ascending ">Male/Female</th>
                            ...
                            <th onclick="return shorting('end_date','{{$order_by}}');" class="sorting   @if(isset($order_type) && $order_type == 'end_date' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'end_date' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " s="" aria-label="Email: activate to sort column ascending ">Status</th>
                            @if(Request::segment(2)=="quota-hall")
                        ...
                        @php  $i=1; @endphp    
                            @foreach ($quotas as $key => $quotasdata)
                                <tr>
                                    ...
                                    <a href="{{route('admin.room.roomDetail',[$quotasdata->id,'show'])}}">@if(isset($quotasdata->created_at) && !empty($quotasdata->created_at)) {{date('Y-m-d H:i:s' , strtotime($quotasdata->created_at))}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.room.roomDetail',[$quotasdata->id,'show'])}}">@if(isset($quotasdata->id) && !empty($quotasdata->id)) #{{$quotasdata->id}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.room.roomDetail',[$quotasdata->id,'show'])}}">@if(isset($quotasdata->quota_id) && !empty($quotasdata->quota_id)) #{{$quotasdata->quota_id}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.room.roomDetail',[$quotasdata->id,'show'])}}">@if(isset($quotasdata->room_code) && !empty($quotasdata->room_code)) {{$quotasdata->room_code}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.room.roomDetail',[$quotasdata->id,'show'])}}">@if(isset($quotasdata->college_name) && !empty($quotasdata->college_name)) {{$quotasdata->college_name}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.room.roomDetail',[$quotasdata->id,'show'])}}">@if(isset($quotasdata->gender) && !empty($quotasdata->gender)) {{$quotasdata->gender}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.room.roomDetail',[$quotasdata->id,'show'])}}">@if(isset($quotasdata->getQuotaHallDetail->start_date) && !empty($quotasdata->getQuotaHallDetail->start_date)) {{date('Y-m-d', $quotasdata->getQuotaHallDetail->start_date)}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.room.roomDetail',[$quotasdata->id,'show'])}}">@if(isset($quotasdata->getQuotaHallDetail->end_date) && !empty($quotasdata->getQuotaHallDetail->end_date)) {{date('Y-m-d',$quotasdata->getQuotaHallDetail->end_date)}} @endif</a></td>
                                   <td valign="top" colspan="1" class="dataTables_empty" >
                                    <div class="select_status_filter">                                            
                                        <span class="badge rounded-pill badge-custom @if(isset($quotasdata->status) && $quotasdata->status == '1') green @else gray @endif">@if($quotasdata->status==1) Enabled @else Disabled @endif </span>
                                    </div>                                      
                                    </td>
                                    @if(Request::segment(2)=="quota-hall")
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                        ...
                                        <i class="ti ti-dots"></i>
                                            ...
                                            <div class="dropdown-menu table-dropdown">
                                                <a class="dropdown-item" href="{{route('admin.room.roomDetail',[$quotasdata->id,'edit'])}}">Edit</a>
