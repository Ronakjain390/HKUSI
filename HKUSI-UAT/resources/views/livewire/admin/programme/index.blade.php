<div class="container-xxl flex-grow-1 container-p-y">
    <div class="card custom-card">
                                                    ...
                                                    <div class="fliter-checkbox">
                                                        <div class="filter-dropdown fx">
                                                            <div class="select-custom" wire:ignore>
                                                                <select id="selectpickerBasic" class="selectpicker w-100" data-style="btn-default" wire:model.defer="programe_code" tabindex="null">
                                                                    <option>Select Programe</option>
                                                                    @if(isset($programe_codedata) && !empty($programe_codedata))
                                                                        @foreach($programe_codedata as $key => $datacode)
                                                                            <option>{{$datacode->programme_code}}</option>
                                                                        @endforeach
                                                                    @endif
                                                                  </select>
                                                            </div>
        ...
        <form method="post" action="{{route('admin.programe.multipleprogramedelete')}}">
        {{ csrf_field() }}
                    ...
                    <div class=" fliter-flex d-flex align-items-center">
                        ...
                        <div class="flex-filter-box" wire:ignore>
                            ...
                            <div class="filter-button fx plus-button">
                                <a href="{{route('admin.programme-setting.create')}}">
                ...
                <table class="dt-select-table table dataTable no-footer dt-checkboxes-select " id="DataTables_Table_3 " aria-describedby="DataTables_Table_3_info ">
                    <thead>
                        <tr>
                            ...
                            <th onclick="return shorting('created_at','{{$order_by}}');" class="sorting @if(isset($order_type) && $order_type == 'created_at' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'created_at' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0 " aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1" aria-sort="descending " aria-label="Name: activate to sort column ascending ">Year</th>
                            ...
                            <th onclick="return shorting('programme_name','{{$order_by}}');" class="sorting   @if(isset($order_type) && $order_type == 'programme_name' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'programme_name' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " s="" aria-label="Programe Name: activate to sort column ascending ">Programme Name</th>
                            <th onclick="return shorting('start_date','{{$order_by}}');" class="sorting   @if(isset($order_type) && $order_type == 'start_date' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'start_date' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " s="" aria-label="Programe Name: activate to sort column ascending ">Start Date</th>
                            <th onclick="return shorting('end_date','{{$order_by}}');" class="sorting   @if(isset($order_type) && $order_type == 'end_date' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'end_date' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " s="" aria-label="Programe Name: activate to sort column ascending ">End Date</th>
                    ...
                    <tbody>
                        @if(count($programme))
                            @foreach($programme as $key => $programeData)
                                <tr>
                                    <td valign="top" colspan="1" class="dt-checkboxes-cell dt-checkboxes-select-all "><input type="checkbox" name='id[]' class="form-check-input" value="{{$programeData->id}}"></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.programmeDetail',[$programeData->id,'details'])}}">@if(isset($programeData->created_at) && !empty($programeData->created_at)) {{date('Y-m-d H:i:s' , strtotime($programeData->created_at))}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.programmeDetail',[$programeData->id,'details'])}}">
                                        @php $year = ''; @endphp
                                        @if (isset($programeData->getProgrammeHallSetting) && count($programeData->getProgrammeHallSetting))
                                            @foreach($programeData->getProgrammeHallSetting as $programmeYear)
                                                @if(!empty($year))
                                    ...
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.programmeDetail',[$programeData->id,'details'])}}">@if(isset($programeData->programme_code) && !empty($programeData->programme_code)) {{$programeData->programme_code}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.programmeDetail',[$programeData->id,'details'])}}">@if(isset($programeData->programme_name) && !empty($programeData->programme_name)) {{$programeData->programme_name}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.programmeDetail',[$programeData->id,'details'])}}">@if(isset($programeData->start_date) && !empty($programeData->start_date)) {{date('Y-m-d' , $programeData->start_date)}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="{{route('admin.programmeDetail',[$programeData->id,'details'])}}">@if(isset($programeData->end_date) && !empty($programeData->end_date)) {{date('Y-m-d' , $programeData->end_date)}} @endif</a></td>
                                     <td valign="top" colspan="1" class="dataTables_empty" >
                                        <div class="select_status_filter">                                            
                                            <span class="badge rounded-pill badge-custom  @if(isset($programeData->status) && $programeData->status == '1') green @else gray @endif"> @if($programeData->status==1) Enabled @else Disabled @endif</span>
                                        </div>                                      
                                    </td>                               
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                        <div class="dropdown text-center">
                                            ...
                                            <div class="dropdown-menu table-dropdown">
                                                <a class="dropdown-item" href="{{route('admin.programmeDetail',[$programeData->id,'edit'])}}">Edit</a>
                                                <a wire:click="$emit('triggerDelete',{{$programeData->id }})" class="dropdown-item" href="javascript:void(0);">Delete</a>     
                                                @if(isset($programeData->status) && $programeData->status == '0')
                                                <a wire:click="$emit('triggerStatus',{{ $programeData->id }})" class="dropdown-item" href="javascript:void(0);">Enable</a>
                                                @else
                                                <a wire:click="$emit('triggerStatus',{{ $programeData->id }})" class="dropdown-item" href="javascript:void(0);">Disable</a>
                                                @endif                                         
                                            </div>
                                        </div>
                                    </td>
