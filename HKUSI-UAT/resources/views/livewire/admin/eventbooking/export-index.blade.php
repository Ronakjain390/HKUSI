                ...
                <table class="dt-select-table table dataTable no-footer dt-checkboxes-select " id="DataTables_Table_3 " aria-describedby="DataTables_Table_3_info ">
                    <thead>
                        <tr>
                            ...
                            <th onclick="return shorting('end_date','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'end_date' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'end_date' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="Salary: activate to sort column ascending ">Time</th>
                            <th onclick="return shorting('start_date','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'start_date' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'start_date' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="Status: activate to sort column ascending ">Location</th>
                            ...
                            <th onclick="return shorting('amount','{{$order_by}}');" class="sorting  @if(isset($order_type) && $order_type == 'amount' && isset($order_by) && $order_by == 'DESC') sorting_desc @elseif(isset($order_type) && $order_type == 'amount' && isset($order_by) && $order_by == 'ASC') sorting_asc @endif " tabindex="0" aria-controls="DataTables_Table_3 " rowspan="1 " colspan="1 " aria-label="Status: activate to sort column ascending ">Assembly Time</th>
                        ...
                        @php  $i=1; @endphp        
                            @foreach ($hallbooking as $key => $eventboookingData)
                                <tr>
                                    ...
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="javascript:void(0);"> @if(isset($eventboookingData->session) && !empty($eventboookingData->session)) {{date('H:i',$eventboookingData->session)}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    ...
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="javascript:void(0);"> @if(isset($eventboookingData->assembly_time) && !empty($eventboookingData->assembly_time)) {{date('H:i',$eventboookingData->assembly_time)}} @endif</a></td>
                                    <td valign="top" colspan="1" class="dataTables_empty">
                                    <a href="javascript:void(0);">
                                   @if(isset($eventboookingData->amount) && !empty($eventboookingData->amount)) {{$eventboookingData->amount}} @endif
                                    </a></td>
                                    <td valign="top" style="pointer-events: none;" colspan="1" class="dataTables_empty">
                                        <div class="select_status_filter">
                                            <select readonly id="" class="selectstatus w-100  @if($eventboookingData->booking_status=='Completed') green @elseif($eventboookingData->booking_status=='Pending') brown  @elseif($eventboookingData->booking_status=='Paid') orange  @elseif($eventboookingData->booking_status=='Cancelled') gray  @elseif($eventboookingData->booking_status=='Updated') red  @endif" data-style="btn-default" tabindex="null" name="status" onchange="return changeStatus(this.value,'{{$eventboookingData->id}}')">  
                                                @if($eventboookingData->booking_status == "Completed")
                                                    <option value="Completed">Completed</option>
                                                    @else
                                                    <option value="Completed">Completed</option>
                                                    @endif
                                                    @if($eventboookingData->booking_status == "Pending")     
                                                    <option value="Pending"selected>Pending</option>
                                                    @else
                                                    <option value="Pending">Pending</option>
                                                    @endif
                                                    @if($eventboookingData->booking_status == "Accepted")     
                                                    <option value="Accepted"selected>Accepted</option>
                                                    @else
                                                    <option value="Accepted">Accepted</option>
                                                    @endif
                                                    @if($eventboookingData->booking_status == "Paid")     
                                                    <option value="Paid"selected>Paid</option>
                                                    @else
                                                    <option value="Paid">Paid</option>
                                                    @endif
                                                    @if($eventboookingData->booking_status == "Cancelled")     
                                                    <option value="Cancelled"selected>Cancelled</option>
                                                    @else
                                                    <option value="Cancelled">Cancelled</option>
                                                    @endif
                                                     @if($eventboookingData->booking_status == "Updated") 
                                                    <option value="Updated" selected>Updated</option>
                                                    @else
                                                    <option value="Updated" >Updated</option>
                                                    @endif
                                                     @if($eventboookingData->booking_status == "Rejected") 
