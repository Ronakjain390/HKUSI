...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="col-6">
                <div class="profile-part">
                    <div class="profile-text">
                        <h4 class="Profile-name">Hall @if(isset($quotaHallInfo->id) && !empty($quotaHallInfo->id)) #{{$quotaHallInfo->id}}@endif</h4>
                        <div class="profilte-btn">
                            <span class="badge rounded-pill badge-custom @if(isset($quotaHallInfo->status) && $quotaHallInfo->status == '1') green @else gray @endif">@if(isset($quotaHallInfo->status) && $quotaHallInfo->status == '1') Release @else Pending @endif</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="profile-tab">
                  ...
                  <i class="ti ti-dots"></i>
                        ...
                        <div class="dropdown-menu table-dropdown">
                            <a class="dropdown-item" href="{{route('admin.quotahall.quotahallDetails',[$quotaHallInfo->id,'edit'])}}">Edit</a>

                            {!! Form::open(['method' => 'DELETE','route' => ['admin.quota.destroy', $quotaHallInfo->id],'style'=>'display:inline','id'=>'delete_form_'.$quotaHallInfo->id]) !!}
                            {!! Form::close() !!}
                            <a class="dropdown-item" onclick="delete_member('{{$quotaHallInfo->id}}')" href="javascript:void()">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="profile-page-buttons-section" id="active">
        <a href="{{route('admin.quotahall.quotahallDetails',[$quotaHallInfo->id,'show'])}}"  class="btn btn-custom @if((isset($dataType) && !empty($dataType) && $dataType == 'show') || (isset($dataType) && !empty($dataType) && $dataType == 'edit'))  active @endif">
        ...
        <a href="{{route('admin.quotahall.quotahallDetails',[$quotaHallInfo->id,'room'])}}">
        ...
        <a href="{{route('admin.quotahall.quotahallDetails',[$quotaHallInfo->id,'programme'])}}">
    ...
    @php $field_name = "quota_hall_id"; @endphp
     ...
     <livewire:admin.quota-hall-programme-management :qouta_hall_id=$dataId />
    @endif
