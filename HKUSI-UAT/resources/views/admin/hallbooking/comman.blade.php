...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="col-6">
                <div class="profile-part">
                    <div class="profile-text">
                        <h4 class="Profile-name">Hall Booking @if(isset($hallBookingInfo->booking_number) && !empty($hallBookingInfo->booking_number)) # {{$hallBookingInfo->booking_number}}@endif </h4>
                        <div class="profilte-btn"><span class="badge rounded-pill badge-custom @if($hallBookingInfo->status=='Completed') green @elseif($hallBookingInfo->status=='Pending') brown  @elseif($hallBookingInfo->status=='Accepted') blue  @elseif($hallBookingInfo->status=='Paid') orange  @elseif($hallBookingInfo->status=='Cancelled') gray  @elseif($hallBookingInfo->status=='Updated') red  @elseif($hallBookingInfo->status=='Rejected') violet @endif ">{{$hallBookingInfo->status}}</span></a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="profile-tab">
                  ...
                  <i class="ti ti-dots"></i>
                        ...
                        <div class="dropdown-menu table-dropdown">
                            <a class="dropdown-item" href="{{route('admin.hallbookingDetails',[$hallBookingInfo->id,'edit'])}}">Edit</a>
                            {!! Form::open(['method' => 'DELETE','route' => ['admin.hallbooking.destroy', $hallBookingInfo->id],'style'=>'display:inline','id'=>'delete_form_'.$hallBookingInfo->id]) !!}
                            {!! Form::close() !!}
                            <a class="dropdown-item" onclick="delete_member('{{$hallBookingInfo->id}}')" href="javascript:void(0)">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="profile-page-buttons-section" id="active">
        <a href="{{route('admin.hallbookingDetails',[$hallBookingInfo->id,'show'])}}"  class="btn btn-custom @if((isset($dataType) && !empty($dataType) && $dataType == 'show') || (isset($dataType) && !empty($dataType) && $dataType == 'edit'))  active @endif">
         ...
         <a href="{{route('admin.hallbookingDetails',[$hallBookingInfo->id,'payment'])}}"  class="btn btn-custom @if(isset($dataType) && !empty($dataType) && $dataType == 'payment') active @endif">
    ...
    @if(isset($dataType) && !empty($dataType) && $dataType == 'edit')
        ...
        @include('admin.hallbooking.edit')

    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'show')

        @include('admin.hallbooking.show')
    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'payment')
	<livewire:admin.payment-management :booking_number=$booking_number />
    @endif
  ...
  Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                ...
                processData: false,
                type: 'POST',
                url: "{!! route('admin.hallbooking.hallbookingstatuschange', [$hallBookingInfo->id]) !!}",
                data:form_data,                               
