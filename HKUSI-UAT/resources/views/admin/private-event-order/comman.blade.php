...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="col-6">
                <div class="profile-part">
                    <div class="profile-text">
                        <h4 class="Profile-name">@if(isset($eventInfo->event_name) && !empty($eventInfo->event_name))  {{$eventInfo->event_name}}@endif @if(isset($bookingInfo->booking_id) && !empty($bookingInfo->booking_id)) # {{$bookingInfo->booking_id}} @endif  </h4>
                        <div class="profilte-btn"><span class="badge rounded-pill badge-custom @if($bookingInfo->booking_status=='Paid') orange @elseif($bookingInfo->booking_status=='Pending') brown  @elseif($bookingInfo->booking_status=='Cancelled') gray @endif" >{{$bookingInfo->booking_status}}</span></a>
                        </div>
                    </div>
                </div>
            </div>
                  ...
                  <i class="ti ti-dots"></i>
                        ...
                        <div class="dropdown-menu table-dropdown">
                            @if(Request::segment(4) == "edit")
                              <a class="dropdown-item" href="{{route('admin.privateEventOrderDetail',[$bookingInfo->id,'show'])}}">View</a>
                            @endif

                            @if(Request::segment(4) == "show")
                              <a class="dropdown-item" href="{{route('admin.privateEventOrderDetail',[$bookingInfo->id,'edit'])}}">Edit</a>
                            @endif

                            {!! Form::open(['method' => 'DELETE','route' => ['admin.private-event-order.destroy', $eventInfo->id],'style'=>'display:inline','id'=>'delete_form_'.$bookingInfo->id]) !!}
                            {!! Form::close() !!}
                            <a class="dropdown-item" onclick="delete_member('{{$bookingInfo->id}}')" href="javascript:void(0)">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="profile-page-buttons-section" id="active">
        <a href="{{route('admin.privateEventOrderDetail',[$bookingInfo->id,'show'])}}"  class="btn btn-custom @if((isset($dataType) && !empty($dataType) && $dataType == 'show') || (isset($dataType) && !empty($dataType) && $dataType == 'edit'))  active @endif">
    ...
    @if(isset($dataType) && !empty($dataType) && $dataType == 'edit')
        @include('admin.private-event-order.edit')
    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'show')
        @include('admin.private-event-order.show')
    @endif
