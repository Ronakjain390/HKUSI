...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="col-6">
                <div class="profile-part">
                    <div class="profile-text">
                        <h4 class="Profile-name">Room @if(isset($quotaRoomInfo->room_code) && !empty($quotaRoomInfo->room_code)) #{{$quotaRoomInfo->room_code}}@endif</h4>
                        <div class="profilte-btn">
                            <span class="badge rounded-pill badge-custom @if(isset($quotaRoomInfo->status) && $quotaRoomInfo->status == '1') green @else gray @endif">@if(isset($quotaRoomInfo->status) && $quotaRoomInfo->status == '1') Enabled @else Disabled @endif</span>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="profile-tab">
                  ...
                  <i class="ti ti-dots"></i>
                        ...
                        <div class="dropdown-menu table-dropdown">
                            <a class="dropdown-item" href="{{route('admin.room.roomDetail',[$quotaRoomInfo->id,'edit'])}}">Edit</a>

                            {!! Form::open(['method' => 'DELETE','route' => ['admin.quota.destroy', $quotaRoomInfo->id],'style'=>'display:inline','id'=>'delete_form_'.$quotaRoomInfo->id]) !!}
                            {!! Form::close() !!}
                            <a class="dropdown-item" onclick="delete_member('{{$quotaRoomInfo->id}}')" href="javascript:void()">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="profile-page-buttons-section" id="active">
        <a href="{{route('admin.room.roomDetail',[$quotaRoomInfo->id,'show'])}}"  class="btn btn-custom @if((isset($dataType) && !empty($dataType) && $dataType == 'show') || (isset($dataType) && !empty($dataType) && $dataType == 'edit'))  active @endif">
        ...
        <a href="{{route('admin.room.roomDetail',[$quotaRoomInfo->id,'record'])}}">
    ...
    @if(isset($dataType) && !empty($dataType) && $dataType == 'show')
        @include('admin.quota-room.show')
    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'edit')
        @include('admin.quota-room.edit')
    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'record')
    <livewire:admin.room-booking-record-management :quota_room_id=$dataId />
     @endif
