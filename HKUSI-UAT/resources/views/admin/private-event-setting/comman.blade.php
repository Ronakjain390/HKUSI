...
@section('content')
{{-- Private event common view created By Akash --}}
<div class="container-xxl flex-grow-1 container-p-y">
                  ...
                  <i class="ti ti-dots"></i>
                        ...
                        <div class="dropdown-menu table-dropdown">
                            @if(Request::segment(4) == "edit")
                              <a class="dropdown-item" href="{{route('admin.privateEventSettingDetails',[$eventInfo->id,'show'])}}">View</a>
                            @endif
                            @if(Request::segment(4) == "images")
                             <a class="dropdown-item" href="{{route('admin.privateEventSettingDetails',[$eventInfo->id,'editimage'])}}">Edit</a>
                             @elseif(Request::segment(4) == "editimage")
                                <a class="dropdown-item" href="{{route('admin.privateEventSettingDetails',[$eventInfo->id,'images'])}}">Images</a>
                             @elseif(Request::segment(4) == "show")
                               <a class="dropdown-item" href="{{route('admin.privateEventSettingDetails',[$eventInfo->id,'edit'])}}">Edit</a>
                             @else
                               <a class="dropdown-item" href="{{route('admin.privateEventSettingDetails',[$eventInfo->id,'edit'])}}">Edit</a>
                             
                           @endif
                            {!! Form::open(['method' => 'DELETE','route' => ['admin.private-event-setting.destroy', $eventInfo->id],'style'=>'display:inline','id'=>'delete_form_'.$eventInfo->id]) !!}
                            {!! Form::close() !!}
    ...
    <div class="profile-page-buttons-section" id="active">
        <a href="{{route('admin.privateEventSettingDetails',[$eventInfo->id,'show'])}}"  class="btn btn-custom @if((isset($dataType) && !empty($dataType) && $dataType == 'show') || (isset($dataType) && !empty($dataType) && $dataType == 'edit'))  active @endif">
         ...
         <a href="{{route('admin.privateEventSettingDetails',[$eventInfo->id,'images'])}}"  class="btn btn-custom @if(isset($dataType) && !empty($dataType) && $dataType == 'images' || $dataType == 'editimage' )  active @endif">
         ...
         <a href="{{route('admin.privateEventSettingDetails',[$eventInfo->id,'programme'])}}">
    ...
    @if(isset($dataType) && !empty($dataType) && $dataType == 'edit')
        @include('admin.private-event-setting.edit')
    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'show')
        @include('admin.private-event-setting.show')
    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'images')
    <livewire:admin.private-event-images-management :event_id=$dataId />
    @elseif(isset($dataType) && !empty($dataType) && $dataType == "editimage")
    @include('admin.private-event-setting.event-images')
    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'programme')
        <livewire:admin.private-event-programme-management :event_id=$dataId />
    @endif
