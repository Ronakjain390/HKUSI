...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="col-6">
                <div class="profile-part">
                    <div class="profile-text">
                        <h4 class="Profile-name">@if(isset($hotelInfo->hotel_name) && !empty($hotelInfo->hotel_name))  {{$hotelInfo->hotel_name}}@endif  </h4>
                        <div class="profilte-btn"><span class="badge rounded-pill badge-custom @if($hotelInfo->status=='Enabled') green @elseif($hotelInfo->status=='Disabled') brown  @endif">{{$hotelInfo->status}}</span></a>
                        </div>
                    </div>
                </div>
            </div>
                  ...
                  <i class="ti ti-dots"></i>
                        ...
                        <div class="dropdown-menu table-dropdown">
                            @if(Request::segment(4) == "edit")
                              <a class="dropdown-item" href="{{route('admin.hotelSettingDetails',[$hotelInfo->id,'show'])}}">View</a>
                            @endif
                            @if(Request::segment(4) == "images")
                             <a class="dropdown-item" href="{{route('admin.hotelSettingDetails',[$hotelInfo->id,'editimage'])}}">Edit</a>
                             @elseif(Request::segment(4) == "editimage")
                                <a class="dropdown-item" href="{{route('admin.hotelSettingDetails',[$hotelInfo->id,'images'])}}">Images</a>
                             @elseif(Request::segment(4) == "edit")
                             @else
                               <a class="dropdown-item" href="{{route('admin.hotelSettingDetails',[$hotelInfo->id,'edit'])}}">Edit</a>
                           @endif
                            {!! Form::open(['method' => 'DELETE','route' => ['admin.hotel-setting.destroy', $hotelInfo->id],'style'=>'display:inline','id'=>'delete_form_'.$hotelInfo->id]) !!}
                            {!! Form::close() !!}
                            <a class="dropdown-item" onclick="delete_member('{{$hotelInfo->id}}')" href="javascript:void(0)">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="profile-page-buttons-section" id="active">
        <a href="{{route('admin.hotelSettingDetails',[$hotelInfo->id,'show'])}}"  class="btn btn-custom @if((isset($dataType) && !empty($dataType) && $dataType == 'show') || (isset($dataType) && !empty($dataType) && $dataType == 'edit'))  active @endif">
         ...
         <a href="{{route('admin.hotelSettingDetails',[$hotelInfo->id,'images'])}}"  class="btn btn-custom @if(isset($dataType) && !empty($dataType) && $dataType == 'images' || $dataType == 'editimage' )  active @endif">
    ...
    @if(isset($dataType) && !empty($dataType) && $dataType == 'edit')
        @include('admin.hotel-setting.edit')
    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'show')
        @include('admin.hotel-setting.show')
    @elseif(isset($dataType) && !empty($dataType) && $dataType == 'images')
    <livewire:admin.hotel-images-management :hotel_id=$dataId />
    @elseif(isset($dataType) && !empty($dataType) && $dataType == "editimage")
    @include('admin.hotel-setting.hotel-images')
    @endif
