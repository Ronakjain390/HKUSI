...
@section('content')
	<div class="container-xxl flex-grow-1 container-p-y">
					...
					<div class="profile-part">
						...
						<div class="profile-text">
							<h4 class="Profile-name">
								iOS Version @if(isset($AppVersionInfo->ios_version) && !empty($AppVersionInfo->ios_version)) {{$AppVersionInfo->ios_version}} @endif | Android Version @if(isset($AppVersionInfo->android_version) && !empty($AppVersionInfo->android_version)) {{$AppVersionInfo->android_version}} @endif
							</h4>

							<div class="profilte-btn">
								<span class="badge rounded-pill badge-custom green">
									Current
								</span>
							</div>
						</div>
					...
					<div class="profile-tab">
						<div class="dropdown">
							...
							<div class="dropdown-menu table-dropdown">
								<a class="dropdown-item" href="{{ route('admin.studentappversion.versionDetail', [$AppVersionInfo->id, 'edit']) }}">Edit</a>
								{!! Form::open([
								    'method' => 'DELETE',
								    'route' => ['admin.studentappversion.destroy', $AppVersionInfo->id],
								    'style' => 'display:inline',
								    'id' => 'delete_form_' . $AppVersionInfo->id,
								]) !!}
								{!! Form::close() !!}
								<a class="dropdown-item" onclick="delete_member('{{ $AppVersionInfo->id }}')" href="javascript:void(0)">Delete</a>
							</div>
						</div>
					</div>
				</div>
		...
		<div class="profile-page-buttons-section" id="active">
			<a href="{{ route('admin.studentappversion.versionDetail', [$AppVersionInfo->id, 'show']) }}"
		...
		@if(isset($dataType) && !empty($dataType) && $dataType == 'edit')
			@include('admin.studentappversion.edit')
		@elseif(isset($dataType) && !empty($dataType) && $dataType == 'show')
			@include('admin.studentappversion.show')
		@endif
