...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="col-6">
                <div class="profile-part">
                    <div class="profile-text">
                        <h4 class="Profile-name">User #@if(isset($UserInfo->id) &&
                            !empty($UserInfo->id)){{$UserInfo->id}}@endif</h4>

                        @if(isset($dataType) && !empty($dataType) && $dataType == 'edit')
                        <div class="selectdiv">
                            <div class="select_status_filter" style="margin-right: 5px;">
                                <select onchange="return checkstatus(this.value , '{{$UserInfo->id}}')" name="status"
                                    class="selectstatus w-100 @if(isset($UserInfo->status) && $UserInfo->status == '1') green @else gray @endif"
                                    data-style="btn-default" tabindex="null" name="status">
                                    <option value="1" @if(isset($UserInfo->status) && $UserInfo->status == '1') selected
                                        @endif>Enabled
                                    </option>
                                    <option value="0" @if(isset($UserInfo->status) && $UserInfo->status == '0') selected
                            ...
                            <div class="select_status_filter">
                                <select id=""
                                    ...
                                    onchange="return checkmemberinfo(this.value , '{{$UserInfo->id}}')" tabindex="null"
                                    name="status">
                                    <option value="1" @if(isset($UserInfo->status) && $UserInfo->status == '1') selected
                                        @endif>Active</option>
                                    <option value="0" @if(isset($UserInfo->status) && $UserInfo->status == '0') selected
                        ...
                        <div class="profilte-btn">
                            <span
                                class="badge rounded-pill badge-custom @if(isset($UserInfo->status) && $UserInfo->status == '1') green @else gray @endif">@if(isset($UserInfo->status)
                                && $UserInfo->status == '1') Enabled @else Disabled @endif</span>
                            <span
                                class="badge rounded-pill badge-custom @if(isset($UserInfo->status) && $UserInfo->status == '1') green @else gray @endif">@if(isset($UserInfo->status)
                                && $UserInfo->status == '1') Active @else Inactive @endif</span>
                        </div>
                        @endif
                    </div>
                </div>
                    ...
                    <div class="dropdown">
                        ...
                        <div class="dropdown-menu table-dropdown">
                            <a class="dropdown-item"
                                href="{{route('admin.userDetails',[$UserInfo->id,'edit'])}}">Edit</a>

                            {!! Form::open(['method' => 'DELETE','route' => ['admin.users.destroy',
                            $UserInfo->id],'style'=>'display:inline','id'=>'delete_form_'.$UserInfo->id]) !!}
                            {!! Form::close() !!}
                            <a class="dropdown-item" onclick="delete_member('{{$UserInfo->id}}')"
                                href="javascript:void(0)">Delete</a>

                            <a class="dropdown-item {!! $UserInfo->status == '1' ? 'success' : 'danger' !!}"
                                onclick="return change_user_status_info(event)"
                                href="{!! route('admin.users.userstatuschange', [$UserInfo->id, ($UserInfo->status=='1' ? '0' : '1')]) !!}"
                                title="{!! $UserInfo->status == '1' ? 'Disable' : 'Enable' !!}">{!! $UserInfo->status ==
                                '1' ? 'Disable' : 'Enable' !!}</a>

                            <a class="dropdown-item {!! $UserInfo->status == '1' ? 'success' : 'danger' !!}"
                                onclick="return change_member_status_info(event)"
                                href="{!! route('admin.users.userstatuschange', [$UserInfo->id, ($UserInfo->status=='1' ? '0' : '1')]) !!}"
                                title="{!! $UserInfo->status == '1' ? 'Inactive' : 'Active' !!}">{!! $UserInfo->status
                                == '1' ? 'Inactive' : 'Active' !!}</a>
    ...
    <div class="profile-page-buttons-section" id="active">
        <a href="{{route('admin.userDetails',[$UserInfo->id,'show'])}}"
            class="btn btn-custom @if((isset($dataType) && !empty($dataType) && $dataType == 'show') || (isset($dataType) && !empty($dataType) && $dataType == 'edit'))  active @endif">
        ...
        <a href="{{route('admin.userDetails',[$UserInfo->id,'account'])}}"
        ...
        <a href="{{route('admin.userDetails',[$UserInfo->id,'settings'])}}"
...
<script>
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    processData: false,
                    type: 'POST',
                    url: "{!! route('admin.users.userselectstatuschange', [$UserInfo->id]) !!}",
                    data: form_data,
    ...
    function checkmemberinfo(value, id) {
        Swal.fire({
            title: "Are you sure?",
            text: "Are you sure change UserInfo status.",
            icon: "warning",
            showCancelButton: true,
            confirmButtonColor: "#3085d6",
            cancelButtonColor: "#d33",
            ...
            if (result.isConfirmed) {
                ...
                $.ajax({
                    ...
                    processData: false,
                    type: 'POST',
                    url: "{!! route('admin.users.userinfostatus', [$UserInfo->id]) !!}",
                    data: form_data,
                    success: function(data) {},
                });
                location.reload();
