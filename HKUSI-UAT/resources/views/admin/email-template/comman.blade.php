...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="col-6">
                <div class="profile-part">
                    <div class="profile-text">
                        <h4 class="Profile-name">@if(isset($EmailTemplate->title) && !empty($EmailTemplate->title))  {{$EmailTemplate->title}}@endif  </h4>
                        <div class="profilte-btn"><span class="badge rounded-pill badge-custom @if($EmailTemplate->status=='1') green  @else gray @endif">@if($EmailTemplate->status == 1) Enable @else Disable @endif</span></a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="profile-tab">
                  ...
                  <i class="ti ti-dots"></i>
                        ...
                        <div class="dropdown-menu table-dropdown">
                               <a class="dropdown-item" href="{{route('admin.emailtemplatesetting',[$EmailTemplate->id,'edit'])}}">Edit</a>
                            {!! Form::open(['method' => 'DELETE','route' => ['admin.email-template.destroy', $EmailTemplate->id],'style'=>'display:inline','id'=>'delete_form_'.$EmailTemplate->id]) !!}
                            {!! Form::close() !!}
                            <a class="dropdown-item" onclick="delete_member('{{$EmailTemplate->id}}')" href="javascript:void(0)">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="profile-page-buttons-section" id="active">
        <a href="{{route('admin.emailtemplatesetting',[$EmailTemplate->id,'show'])}}"  class="btn btn-custom @if((isset($dataType) && !empty($dataType) && $dataType == 'show') || $dataType == 'edit'))  active @endif">
    ...
    @if(isset($dataType) && !empty($dataType) && $dataType == 'edit')
        @include('admin.email-template.edit')
    @endif
            ...
            <table class="table">
                <tbody>
                    <tr>
                        ...
                        <td>@if(isset($EmailTemplate->created_at) && !empty($EmailTemplate->created_at)) {{date('Y-m-d' , strtotime($EmailTemplate->created_at))}} @endif</td>
                    </tr>
                    <tr>
                        <th class="t-basic">Ttile</th>
                        <td>@if(isset($EmailTemplate->title) && !empty($EmailTemplate->title)) {{$EmailTemplate->title}} @endif</td>
                    </tr>
                </tbody>
            </table>
        </div>
            ...
            <table class="table">
                <tbody>
                    <tr>
                        ...
                        <td>@if(isset($EmailTemplate->description) && !empty($EmailTemplate->description)) {!!nl2br($EmailTemplate->description) !!} @endif</td>
                    </tr>
                </tbody>
            </table>
        </div>
