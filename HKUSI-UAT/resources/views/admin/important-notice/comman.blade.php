...
<div class="container-xxl flex-grow-1 container-p-y">
            ...
            <div class="col-6">
                <div class="profile-part">
                    <div class="profile-text">
                        <h4 class="Profile-name">@if(isset($ImportantNotice->title) && !empty($ImportantNotice->title))  {{$ImportantNotice->title}}@endif  </h4>
                        <div class="profilte-btn"><span class="badge rounded-pill badge-custom @if($ImportantNotice->status=='1') green  @else gray @endif">@if($ImportantNotice->status == 1) Enable @else Disable @endif</span></a>
                        </div>
                    </div>
                </div>
            </div>
                ...
                <div class="profile-tab">
                  ...
                  <i class="ti ti-dots"></i>
                        ...
                        <div class="dropdown-menu table-dropdown">
                               <a class="dropdown-item" href="{{route('admin.importantnoticesetting',[$ImportantNotice->id,'edit'])}}">Edit</a>
                            {!! Form::open(['method' => 'DELETE','route' => ['admin.importantnotice.destroy', $ImportantNotice->id],'style'=>'display:inline','id'=>'delete_form_'.$ImportantNotice->id]) !!}
                            {!! Form::close() !!}
                            <a class="dropdown-item" onclick="delete_member('{{$ImportantNotice->id}}')" href="javascript:void(0)">Delete</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div class="profile-page-buttons-section" id="active">
        <a href="{{route('admin.importantnoticesetting',[$ImportantNotice->id,'show'])}}"  class="btn btn-custom @if((isset($dataType) && !empty($dataType) && $dataType == 'show') || $dataType == 'edit'))  active @endif">
    ...
    @if(isset($dataType) && !empty($dataType) && $dataType == 'edit')
        @include('admin.important-notice.edit')
    @endif
      @if(isset($dataType) && !empty($dataType) && $dataType == 'show')
            ...
            <table class="table">
                <tbody>
                    <tr>
                        ...
                        <td>@if(isset($ImportantNotice->created_at) && !empty($ImportantNotice->created_at)) {{date('Y-m-d' , strtotime($ImportantNotice->created_at))}} @endif</td>
                    </tr>
                    <tr>
                        <th class="t-basic">Ttile</th>
                        <td>@if(isset($ImportantNotice->title) && !empty($ImportantNotice->title)) {{$ImportantNotice->title}} @endif</td>
                    </tr>
                </tbody>
            </table>
        </div>
            ...
            <table class="table">
                <tbody>
                    <tr>
                        ...
                        <td>@if(isset($ImportantNotice->description) && !empty($ImportantNotice->description)) {{strip_tags($ImportantNotice->description)}} @endif</td>
                    </tr>
                </tbody>
            </table>
        </div>
