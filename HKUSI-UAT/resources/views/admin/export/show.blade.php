...
@section('content')
    <div class="container-xxl flex-grow-1 container-p-y">
                ...
                <div class="col-6">
                    <div class="profile-part">
                        <div class="profile-text">
                            <h4 class="Profile-name">Export / #{{$dataId}}</h4>
                            <div class="profilte-btn">
                                <span class="badge rounded-pill badge-custom @if(isset($exportDataInfo->status) && $exportDataInfo->status == '1') green @else gray @endif">@if(isset($exportDataInfo->status) && $exportDataInfo->status == '1') Success @else Failed @endif</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="profile-tab">
                      ...
                      <i class="ti ti-dots"></i>
                            ...
                            <div class="dropdown-menu table-dropdown">
                                {!! Form::open(['method' => 'DELETE','route' => ['admin.export.destroy', $exportDataInfo->id],'style'=>'display:inline','id'=>'delete_form_'.$exportDataInfo->id]) !!}
                                {!! Form::close() !!}
                                <a class="dropdown-item" href="javascript:void(0);" onclick="delete_info('{{$exportDataInfo->id}}')" title="Delete">Delete</a>
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="profile-page-buttons-section">
            <a href="{{route('admin.export.exportDetail',[$exportDataInfo->id,'detail'])}}">
            ...
            <a href="{{route('admin.export.exportDetail',[$exportDataInfo->id,'record'])}}">
                <button type="button" class="btn btn-custom  @if(isset($dataType) && !empty($dataType) && $dataType == 'record') active @endif">
                      <span><svg width="16" height="18" viewBox="0 0 16 18" fill="none" xmlns="http://www.w3.org/2000/svg">
                        ...
                        </span>Records({{$exportDataInfo['type']}})
                </button>
            </a>
        </div>
        @if(isset($dataType) && !empty($dataType) && $dataType == 'record')
            @if(isset($exportDataInfo['type']) && !empty($exportDataInfo['type']) && $exportDataInfo['type'] == 'Payment')
            <livewire:admin.export-payment-management :export_data_info_id=$dataId />
            @elseif(isset($exportDataInfo['type']) && !empty($exportDataInfo['type']) && $exportDataInfo['type'] == 'HallBooking')
            <livewire:admin.export-hall-booking-management :export_data_info_id=$dataId />
            @elseif(isset($exportDataInfo['type']) && !empty($exportDataInfo['type']) && $exportDataInfo['type'] == 'Programme')
            <livewire:admin.import-programme-detail-management :export_data_info_id=$dataId />
            @elseif(isset($exportDataInfo['type']) && !empty($exportDataInfo['type']) && $exportDataInfo['type'] == 'Country')
            <livewire:admin.import-country-detail-management :export_data_info_id=$dataId />
            @elseif(isset($exportDataInfo['type']) && !empty($exportDataInfo['type']) && $exportDataInfo['type'] == 'EventBooking')
            <livewire:admin.export-event-booking-management :export_data_info_id=$dataId />
            @elseif(isset($exportDataInfo['type']) && !empty($exportDataInfo['type']) && $exportDataInfo['type'] == 'PrivateEventBooking')
            <livewire:admin.export-private-event-booking-management :export_data_info_id=$dataId />
            @endif

        @else
            <div class="card custom-card profile-details margin-b-20">
                <div class="basic-details">
                    <h6 class="card-heading">Export Info</h6>
                </div>
                    ...
                    <table class="table">
                        <tbody>
                            <tr>
                                ...
                                <td>@if(isset($exportDataInfo->created_at) && !empty($exportDataInfo->created_at)) {{date('Y-m-d' , strtotime($exportDataInfo->created_at))}} @endif</td>
                            </tr>
                            <tr>
                                <th class="t-basic">Create Time</th>
                                <td>@if(isset($exportDataInfo->created_at) && !empty($exportDataInfo->created_at)) {{date('h:i:s' , strtotime($exportDataInfo->created_at))}} @endif</td>
                            </tr>
                            <tr>
                                <th class="t-basic">Export #</th>
                                <td>@if(isset($exportDataInfo->id) && !empty($exportDataInfo->id)) #{{$exportDataInfo->id}} @endif</td>
                            </tr>
                            <tr>
                                <th class="t-basic">Type</th>
                                <td>@if(isset($exportDataInfo->type) && !empty($exportDataInfo->type)) {{$exportDataInfo->type}} @endif</td>
                            </tr>
                            <tr>
                                <th class="t-basic">Admin User</th>
                                <td>@if(isset($exportDataInfo->getUserDetail->name) && !empty($exportDataInfo->getUserDetail->name)) {{$exportDataInfo->getUserDetail->name}} @endif</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
