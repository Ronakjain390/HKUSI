...
class ImportCountryDetailManagement extends Component
{
    use WithPagination;
    ...
    {
        $programme = (new ImportCountry)->newQuery();
        ...
        $this->totalEnabled =  ImportCountry::select('id','status')->where('import_data_info_id',$this->import_data_info_id)->where('status','1')->count();
        $this->totalDisabled = ImportCountry::select('id','status')->where('import_data_info_id',$this->import_data_info_id)->where('status','0')->count();
        $this->programe_codedata = ImportCountry::orderBy('id','ASC')->where('import_data_info_id',$this->import_data_info_id)->get();
        $this->countPrograme = ImportCountry::where('import_data_info_id',$this->import_data_info_id)->count();
        return view('livewire.admin.country.import-index',compact('programme'));
    }
