...
class RoomBookingRecordManagement extends Component
{
    use WithPagination;
    ...
    public function render()
    {
        ...
        $hallbooking = $hallbooking->where('quota_room_id',$this->quota_room_id)->paginate($this->paginate);
        $this->Completed = HallBookingInfo::where('quota_room_id',$this->quota_room_id)->where('status','Completed')->count();
        $this->Pending = HallBookingInfo::where('quota_room_id',$this->quota_room_id)->where('status','Pending')->count();
        $this->Accepted = HallBookingInfo::where('quota_room_id',$this->quota_room_id)->where('status','Accepted')->count();
        $this->Paid = HallBookingInfo::where('quota_room_id',$this->quota_room_id)->where('status','Paid')->count();
        $this->Cancelled = HallBookingInfo::where('quota_room_id',$this->quota_room_id)->where('status','Cancelled')->count();
        $this->Updated = HallBookingInfo::where('quota_room_id',$this->quota_room_id)->where('status','Updated')->count();
        $this->Rejected = HallBookingInfo::where('quota_room_id',$this->quota_room_id)->where('status','Rejected')->count();
        $this->countMember = HallBookingInfo::count();
        $this->countries = Country::where('status','1')->orderBy('name','ASC')->get();
        return view('livewire.admin.quota-room.records',compact('hallbooking'));
    }
