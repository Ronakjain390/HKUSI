...
class ImportHallDetailsManagement extends Component
{
    use WithPagination;
    ...
    {
        $members = (new ImportHallDetail)->newQuery();
        ...
        $this->totalCompleted = ImportHallDetail::where('import_data_info_id',$this->import_data_info_id)->where('status','1')->count();
        $this->totalFailed = ImportHallDetail::where('import_data_info_id',$this->import_data_info_id)->where('status','0')->count();
        }
        ...
        $this->countMember = ImportHallDetail::where('import_data_info_id',$this->import_data_info_id)->count();
        $this->countries = Country::where('status','1')->orderBy('name','ASC')->get();
        return view('livewire.admin.quota-hall.import-index',compact('members'));
    }
