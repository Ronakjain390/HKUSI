...
class ImportEventManagement extends Component
{
	use WithFileUploads;
    ...
    public function render(){
        ...
        return view('livewire.admin.import.event-import');
    }
    ...
    public function importFrist(){
	    ...
	    File::makeDirectory(base_path('public/storage/events'), 0777, true, true);
    	$file = $this->xlsxFile->store('events', 'public');
    	$this->filePath = base_path('public/storage/').$file;
    	$meberFilds = new EventSetting();
    ...
    public function importSecond(){
        ...
        $importData['type'] = 'Event';
        $importData['status'] = '1';
        $importData->save();
        
        Excel::import(new ImportEvent($this->mappingRow,$this->headerRow[0][0],$importData->id,$this->year), $this->filePath);

        $allImportRecord = ImportEventDetail::where('import_data_info_id',$importData->id)->count();
        $allImportFailedRecord = ImportEventDetail::where('status','0')->where('import_data_info_id',$importData->id)->count();
