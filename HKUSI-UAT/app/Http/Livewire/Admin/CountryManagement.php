...
class CountryManagement extends Component
{
   ...
   public function updatingSearch()
    ...
    {
        $programme = (new Country)->newQuery();
        ...
        $this->totalEnabled =  Country::select('id','status')->where('status','1')->count();
        $this->totalDisabled = Country::select('id','status')->where('status','0')->count();
        $this->programe_codedata = Country::orderBy('id','ASC')->get();
        $this->countMember = Country::count();
        return view('livewire.admin.country.index',compact('programme'));
    }
