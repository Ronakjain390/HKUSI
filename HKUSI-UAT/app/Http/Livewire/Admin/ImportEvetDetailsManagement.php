...
class ImportEvetDetailsManagement extends Component
{
    use WithPagination;
    ...
    {
        $members = (new ImportEventDetail)->newQuery();
        ...
        $this->totalCompleted = ImportEventDetail::where('import_data_info_id',$this->import_data_info_id)->where('status','1')->count();
        $this->totalFailed = ImportEventDetail::where('import_data_info_id',$this->import_data_info_id)->where('status','0')->count();
        }
        ...
        $this->countMember = ImportEventDetail::where('import_data_info_id',$this->import_data_info_id)->count();
        $this->countries = Country::where('status','1')->orderBy('name','ASC')->get();
        return view('livewire.admin.event-setting.import-index',compact('members'));
    }
