...
class EventProgrammeManagement extends Component
{
    use WithPagination;
    ...
    {
        $eventPrograme = (new EventProgramme)->newQuery();
        ...
        $getprogrammeid = EventProgramme::where('event_id',$this->event_id)->pluck('program_id')->toArray();
         // dd($eventPrograme);
        ...
        return view('livewire.admin.event-programe.index',compact('eventPrograme'));
    }

    public function stautsMemberProgrameChange($id) 
    {   
        $status = EventProgramme::find($id); 
        if (empty($id)) {
    ...
    {
        $eventPrograme =EventProgramme::where('event_id',$this->event_id)->where('program_id',$id)->first();
