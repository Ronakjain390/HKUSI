...
class ImportPrivateEventSettingDetailsManagement extends Component
{
    use WithPagination;
    // Import Private Event Setting detail Management By Akash
    ...
    {
        $members = (new ImportPrivateEventDetail)->newQuery();
        ...
        $this->totalCompleted = ImportPrivateEventDetail::where('import_data_info_id',$this->import_data_info_id)->where('status','1')->count();
        $this->totalFailed = ImportPrivateEventDetail::where('import_data_info_id',$this->import_data_info_id)->where('status','0')->count();
        }
        ...
        $this->countMember = ImportPrivateEventDetail::where('import_data_info_id',$this->import_data_info_id)->count();
        $this->countries = Country::where('status','1')->orderBy('name','ASC')->get();
        return view('livewire.admin.private-event-setting.import-index',compact('members'));
    }
