...
class ImportMemberDetailManagement extends Component
{
    use WithPagination;
    ...
    {
        $members = (new ImportMemberDetail)->newQuery();
        ...
        $this->totalCompleted = ImportMemberDetail::where('import_data_info_id',$this->import_data_info_id)->where('status','1')->count();
        $this->totalFailed = ImportMemberDetail::where('import_data_info_id',$this->import_data_info_id)->where('status','0')->count();
        }
        ...
        $this->countMember = ImportMemberDetail::where('import_data_info_id',$this->import_data_info_id)->count();
        $this->countries = Country::where('status','1')->orderBy('name','ASC')->get();
        return view('livewire.admin.members.import-index',compact('members'));
    }
