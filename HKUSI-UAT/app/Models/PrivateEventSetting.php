...
class PrivateEventSetting extends Model
{
    ...
    public function getProgrammeDetailMany(){
        return $this->hasMany(PrivateEventProgramme::class , 'event_id' ,'id');
    }
    ...
    public function getEventImages(){
        return $this->hasMany(PrivateEventSettingImage::class ,'event_setting_id', 'id');
    }
    ...
    public function checkMemberEvent($application_id=''){
        $envetnBook = PrivateEventOrder::where('application_id',$application_id)->where('booking_status','!=','Cancelled')->where('event_id',$this->id)->first();
        return $envetnBook;
    }

    public function getBookingsQouta(){
        return $this->hasMany(PrivateEventOrder::class,'event_id','id')->whereIn('booking_status',['Paid','Updated','Pending','Completed'])->sum('no_of_seats');
    }

    public function getBookings()
    {
        return $this->hasMany(PrivateEventOrder::class, 'event_id', 'id');
    }
