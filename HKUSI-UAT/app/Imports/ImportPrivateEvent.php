...
class ImportPrivateEvent implements ToCollection  ,WithHeadingRow 
{
    ...
    /**
    * @param array $row
    * Import Private Event Created by Aakash
        ...
        if(!empty($rows)){
            foreach ($rows as $keymember => $valuemamber){ 
                ...
                if(isset($this->mappingRow[0]) && !empty($this->mappingRow[0]) 
                    && isset($this->mappingRow[3]) && !empty($this->mappingRow[3]) 
                    && isset($this->mappingRow[5]) && !empty($this->mappingRow[5]) 
                    && isset($this->mappingRow[6]) && !empty($this->mappingRow[6]) 
                    && isset($this->mappingRow[7]) && !empty($this->mappingRow[7]) 
                    && isset($this->mappingRow[8]) && !empty($this->mappingRow[8]) 
                    && isset($this->mappingRow[9]) && !empty($this->mappingRow[9]) 

                    && isset($this->mappingRow[13]) && !empty($this->mappingRow[13]) 
                     && isset($this->mappingRow[14])  && !empty($this->mappingRow[14]) 
                     && isset($this->mappingRow[15]) && !empty($this->mappingRow[15]) 
                     && isset($this->mappingRow[16]) && !empty($this->mappingRow[16]) &&

                     !empty($valuemamber[$this->mappingRow[0]]) 
                     && !empty($valuemamber[$this->mappingRow[3]]) 
                     && !empty($valuemamber[$this->mappingRow[5]]) 
                     && !empty($valuemamber[$this->mappingRow[6]]) 
                     && !empty($valuemamber[$this->mappingRow[7]]) 
                     && !empty($valuemamber[$this->mappingRow[8]]) 
                     && !empty($valuemamber[$this->mappingRow[9]]) 
                     && !empty($valuemamber[$this->mappingRow[10]]) 
                     && !empty($valuemamber[$this->mappingRow[13]]) 
                     && !empty($valuemamber[$this->mappingRow[14]]) 
                     && !empty($valuemamber[$this->mappingRow[15]]) 
                     && !empty($valuemamber[$this->mappingRow[16]])){

                    $EventSetting = PrivateEventSetting::where('event_name',$valuemamber[$this->mappingRow[0]])->first();
                    if (empty($EventSetting)) {
                        $EventSettingData = new PrivateEventSetting();
                        ...
                        if(isset($databaseFields) && count($databaseFields)){
                            ...
                            $EventSettingData['quota_balance'] = $valuemamber[$this->mappingRow[10]];
                            foreach($databaseFields as $key => $getFieldsData){
                                if(isset($this->mappingRow[$key]) && !empty($this->mappingRow[$key])){
                                    if($getFieldsData=='date'){
                                        ...
                                        $assembly_time = $valuemamber[$this->mappingRow[5]];
                                        $EventSettingData[$getFieldsData] = $assembly_time;
                                    }elseif($getFieldsData=='assembly_end_time'){
                                        $assembly_time = $valuemamber[$this->mappingRow[6]];
                                        $EventSettingData[$getFieldsData] = $assembly_time;
                        ...
                        if ( isset($this->mappingRow[23]) && !empty($this->mappingRow[23]) && !empty($valuemamber[$this->mappingRow[23]])) {
                            ...
                            if ( !empty($explodedata)) {
                                foreach($explodedata as $datagetPrograme){
                                    $programmes = Programme::where('application_number',$datagetPrograme)->first();
                                    if(!empty($programmes)){
                                        PrivateEventProgramme::insert(['event_id'=>$event_id,'program_id'=>$programmes->id]);
                                    }
                                }
                            }
                        }
                        ...
                        /** Import Data**/ 
                        $importEventDetails = new ImportPrivateEventDetail();
                            ...
                            foreach($databaseFields as $key => $getTabledatavalues){
                                if(isset($this->mappingRow[$key]) && !empty($this->mappingRow[$key])){
                                    if($getTabledatavalues=='date'){
                                        ...
                                        $assembly_time = $valuemamber[$this->mappingRow[5]];
                                        $importEventDetails[$getTabledatavalues] = $assembly_time;
                    ...
                    /** End **/                     
                        ...
                        if (!empty($EventSetting)) {
                            $failedMemberData = new ImportPrivateEventDetail();
                                ...
                                foreach($databaseFields as $key => $getTableFieldsvalue){
                                    if(isset($this->mappingRow[$key]) && !empty($this->mappingRow[$key])){
                                        if($getTableFieldsvalue=='date'){
                                            ...
                                            $assembly_time = $valuemamber[$this->mappingRow[5]];
                                            $failedMemberData[$getTableFieldsvalue] = $assembly_time;
                                ...
                                foreach($databaseFields as $key => $getTableFieldsvalus){
                                    if(isset($this->mappingRow[$key]) && !empty($this->mappingRow[$key])){
                                        if($getTableFieldsvalus=='date'){
                                            ...
                                            $assembly_time = $valuemamber[$this->mappingRow[5]];
                                            $failedMemberData[$getTableFieldsvalus] = $assembly_time;
                    ...
                    if(!empty($valuemamber[$this->mappingRow[0]])){
                        ...
                        if(isset($databaseFields) && count($databaseFields)){
                            ...
                            }elseif(empty($valuemamber[$this->mappingRow[10]])){
                                $failedMemberData['reason'] = 'Event qouta empty.';
                            }elseif(empty($valuemamber[$this->mappingRow[13]])){
                            ...
                            }elseif(empty($valuemamber[$this->mappingRow[15]])){
